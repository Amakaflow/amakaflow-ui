...
interface MappingResolutionCardProps {
  ...
  onResolve: (exerciseId: string, mappedName: string) => void;
}

export function MappingResolutionCard({
  exerciseId,
  exerciseName,
  suggestions,
  onResolve,
}: MappingResolutionCardProps) {
  ...
  const handleAccept = (suggestion: string) => {
    onResolve(exerciseId, suggestion);
    setResolved(true);
    setOpen(false);
  };
  ...
  if (resolved) {
    return (
      <div className="flex items-center gap-2 px-3 py-2 rounded-md bg-muted/50 text-sm">
        ...
        <span className="flex-1 truncate text-muted-foreground">{exerciseName}</span>
  ...
  return (
    <div className="border rounded-md">
      ...
      >
        ...
        <span className="flex-1 text-sm truncate">{exerciseName}</span>
        {topSuggestion && (
