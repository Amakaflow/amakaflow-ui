...
class TestGenerationAPIEndpoints:
    ...
    Currently returns 501 until implementation in AMA-462.
    """
    ...
    @pytest.mark.xfail(reason="POST /generate returns 501 until AMA-462 implementation")
    def test_generate_program(
        ...
        request_data = {
            ...
            "duration_weeks": 4,
            "sessions_per_week": 3,
            "equipment_available": ["barbell", "dumbbell", "pull_up_bar"],
        }
        ...
        if result.get("program"):
            cleanup_program(result["program"]["id"])

        assert "program" in result or "message" in result
