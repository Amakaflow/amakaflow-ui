...
class AsyncFunctionDispatcher:
    ...
    async def _import_workout_from_url(
        self, args: Dict[str, Any], ctx: FunctionContext
    ) -> str:
        """Import a workout from any supported URL (auto-detects platform).

        Delegates to the platform-specific import handlers. This is a unified
        entry point that auto-detects YouTube, TikTok, Instagram, or Pinterest.
        """
        url = args.get("url")
        if not url:
            return self._error_response(
        ...
        return await handler({"url": url, **{k: v for k, v in args.items() if k != "url"}}, ctx)
